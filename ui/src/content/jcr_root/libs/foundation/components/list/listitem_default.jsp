...
%><li>
    <p>
        <a href="<%= listItem.getPath() %>.html" title="<%= StringEscapeUtils.escapeHtml4(listItem.getTitle()) %>"
           onclick="CQ_Analytics.record({event: 'listItemClicked', values: { listItemPath: '<%= listItem.getPath() %>' }, collect:  false, options: { obj: this }})"><%= StringEscapeUtils.escapeHtml4(listItem.getTitle()) %></a>
    </p>
</li>