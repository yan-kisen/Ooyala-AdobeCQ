    ...
    <%
        ...
        <div style="padding: 0px 0px 0px 15px; width: 522px; float:left;"> 
            <div style="padding: 2px 0px 2px 0px;  border-bottom: 1px solid #f5f5f5; height: 18px; background-color: #f9f9f9;">
                <div style="padding-left: 12px; width: 250px; float: left;">Service is</div>
                <div style="padding-left: 10px; width: 100px; float: left;"><strong><%= status %></strong></div>
            </div> 
            <%
        ...
        <div class="available-configs">
            <h2 style="border: none; margin-top: 10px; padding-left:0px;">Available Configurations
            <% String resPath = resource.getPath().replace("/jcr:content", ""); %> 
            [<a href="javascript: CQ.cloudservices.editNewConfiguration('<%=resPath%>','<%=resPath%>', true, 'Create Configuration')" style="color: #336600;" title="Add new configuration"><b>+</b></a>]
            </h2>
            <%@include file="/libs/cq/cloudserviceconfigs/components/childlist/childlist.jsp"%><%-- cq:include accepts no params --%>
            <%=printChildren(currentPage)%>
        </div> 
</body>
</html>
